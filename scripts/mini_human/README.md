...
seq 1 12 | while read i; do
  sbatch -p workers -c 48 --job-name tomato --wrap "hostname; \time -v bash /lizardfs/guarracino/wfmash-paper/scripts/mini_tomato/single_human_chr.sh $s $l $p $n $i $PATH_WFMASH $PREFIX $DIR_OUTPUT"
done
```
