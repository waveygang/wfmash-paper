...
%\end{figure}
    ...
    We evaluated the performance of \wfmash\ 0.xx against Minimap2-v2.20-r1061 {29750242} and Winnomap 2.03 {XXX}, in
    ...
    (Table 1 and Table 2). Chromosomes were split into shorter sequences using the splitfa tool~\citep{splitfa}. In each
    run, all sequences have the same length. Single nucleotide variants (SNVs), small indels, and structural
    variants (SVs) were introduced using the Mutation-Simulator script~\citep{32780803}.
    ...
    a pangenome graph in each run, and called the variants on the graphs. The graphs were built using~\citep{seqwish}.
    ...
    evaluating the false-negative and false-positive rates (FNR, FPR) using vcfeval~\citep{vcfeval}.
