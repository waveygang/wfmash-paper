...
```bash
minimap2 -ax map-hifi -t {threads} {input.ref} {input.fastq}|{prefix}/samtools sort -@ {threads} -O bam -o {output.bam}
...
mamba install -c conda-forge -c bioconda wfmash=0.12.5 anchorwave=1.2.1 minimap2=2.26 bcftools samtools=1.16 syri=1.6.3 mummer4=4.0.0rc1 cutesv=2.1.0
...
for p in 80 90 95
do
    #$time -f "$fmt" wfmash -s 10000 -p $p -n 1 -k 19 -H 0.001 -Y '#' -t 36 --hg-filter-ani-diff 30 Col-CC.fa ${sample}.fa > ${sample}.wfmash_p${p}s10kn1k19.paf
    $time -f "$fmt" wfmash -a -s 10000 -p $p -n 1 -k 19 -H 0.001 -Y '#' -t 36 --hg-filter-ani-diff 30 Col-CC.panSN.fa.gz ${sample}.panSN.fa.gz > ${sample}.wfmash_p${p}s10kn1k19.sam
done
...
for p in asm5 asm20
do
#    #$time -f "$fmt" minimap2 -x $p -c --eqx -t 36 Col-CC.fa ${sample}.fa > ${sample}.mm2_${p}.paf
    $time -f "$fmt" minimap2 -ax $p -c --eqx -t 36 Col-CC.fa ${sample}.fa > ${sample}.mm2_${p}.sam
done
...
## anchorwave
$time -f "$fmt" minimap2 -x splice -t 10 -k 12 -a -p 0.4 -N 20 {input.fa} Col-CC.filter.cds.fa > {output.sam}
$time -f "$fmt" anchorwave genoAli -i ${ref}.gene.gff -as {input.cds} -r {input.ref} -a {output.sam} -ar {input.sam} -s {input.fa} -n {output.anchor} -o {output.maf} -f {params.mmaf} -IV

# mummmer
$time -f "$fmt" nucmer --sam-long ${sample}.nucmer.sam --maxmatch -c 500 -b 500 -l 100 -t 8 Col-CC.fa ${sample}.fa
...
```shell
# combine sam into one bam

grep "^@" ${sample}.mm2_asm20.sam > header

for i in `ls *.sam|sed -e "s/.sam//g" -e "s/${sample}.//g"`;
do
    echo "$i"
    if [[ $i == *aw* ]]; then
        ...
        sed -e "s/${sample}#1#/${sample}#${i}#1#/g" -e "s/Col-CC#1#//g" ${sample}.${i}.sam |grep -v "^@" >> combine.sam
        sed "s/Col-CC#1#//g" ${sample}.${i}.sam|samtools sort -@ 12 -O bam -o ${sample}.${i}.sorted.bam -
    elif [[ $i == *mm2* ]];then
