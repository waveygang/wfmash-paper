...
WFMASH=/lizardfs/guarracino/git/wfmash/build/bin/wfmash-08d8873b61b4908db4afa76481c45b0227067284
...
IMPG=/lizardfs/guarracino/git/impg/target/release/impg-ac5088a0fd12684dbceffb43722ccdacf7ac2d07
...
FASTA=$DIR_BASE/assemblies/scerevisiae/scerevisiae7.fa.gz
NAME=$(basename $FASTA .fa.gz)
DIR_OUTPUT=$DIR_BASE/alignment/scerevisiae
...
cut -f 1,2 $FASTA.fai -d '#' | sort | uniq | while read TARGET; do
    ...
    cut -f 4 $DIR_OUTPUT/scerevisiae.busco-genes.single.bed | sort | uniq | while read GENE; do
        ...
        bedtools subtract \
            ...
            -b <($IMPG -p $PAF -b <(grep "^$TARGET" $DIR_OUTPUT/scerevisiae.busco-genes.single.bed | grep $GENE -w | bedtools slop -b 5000 -g <(cut -f 1,2 $FASTA.fai)) | bedtools sort)
