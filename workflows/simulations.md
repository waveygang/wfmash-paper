...
```shell
(echo wfmash vcfrandomsample minimap2 mutation-simulator.py | tr ' ' '\n') | while read tool; do ls -l $(which $tool); done | cut -f 13 -d ' '
...
Sample generation: # TODO split MULTIFASTA in single FASTA, sample mutations, then merge
...
do
    ...
    for index_s in ${!samples[*]}; do
      ...
      cut -f 1 $path_mutated_base_fa.fai | while read seq_name; do
        len_input_fasta=$(grep $seq_name $path_input_fasta.fai | cut -f2)
      	num_mutations_vcf=$(grep '^#' $path_mutations_vcf -v | grep $seq_name -c);
        ...
        vcflib vcfrandomsample <(cat <(grep '^#' $path_mutations_vcf) <(grep ^$seq_name $path_mutations_vcf)) --rate $divergence_rate | grep '^#' -v >> samples/$name_output.$samplename.tmp.vcf
      done
      
      cat <(grep '^#' $path_mutations_vcf | sed "s/FORMAT\t${name_input_fasta}_mt/FORMAT\t${samplename}/g") <(grep '^#' -v samples/$name_output.$samplename.tmp.vcf) | bgzip -c > samples/$name_output.$samplename.vcf.gz;
...
do
	...
	for index_s in ${!samples[*]}; do
	  ...
	  $run_fastix -p "${samplename}#" <(cat $path_input_fasta | bcftools consensus $path_sample_mutations_vcf_gz --sample $samplename) >> samples/$name_output.samples.fa
	done
done
...
do
	...
	for idx_len in ${!lengths[*]}; do
	  len=${lengths[$idx_len]};
	  cat $path_input_fasta <($run_splitfa $path_input_mt_fasta -l $len-$len -s 1 2> assemblies/samples_$divergence.l${len}.paf | perl /home/guarracino/Desktop/SharedFolder/wfmash-paper/scripts/split_contigs.perl -) | bgzip -@ $threads -c > assemblies/$base_output.l${len}.fa.gz;
	  samtools faidx assemblies/$base_output.l${len}.fa.gz;
	  #??? sed 's/sample.#chr8_b_def\t/chr8_b_def\t/g' assemblies/samples_$divergence.l${len}.paf -i;
	done;
done
...
do
	...
	for idx_len in ${!lengths[*]}; do
	  ...
	  echo $path_paf;
	  \time -v minimap2 $path_input_fa_gz $path_input_fa_gz -t $threads -c -x $preset -X 2>> alignments/minimap2/minimap2.log > $path_paf;
	  
	  # Filter short alignments
	  for l in 100000 400000 800000; do
	    cat $path_paf | fpa drop -l $l > alignments/minimap2/${name_input_fasta}+samples_$divergence.l${len}.drop$l.paf;
	  done;
	done;
done
```
...
do
	...
	for idx_len in ${!lengths[*]}; do len=${lengths[$idx_len]};
        ...
        echo $path_paf;
        \time -v wfmash $path_input_fa_gz $path_input_fa_gz -t $threads -s 100k -l 300k -p $identity -X -n $num_haplotypes 2>> alignments/wfmash/wfmash.log > $path_paf;
	done;
done
```
...
do
	...
	for aligner in minimap2 winnomap2; do
	  ...
	  for idx_len in ${!lengths[*]}; do
	    len=${lengths[$idx_len]};
	    path_input_fa_gz=assemblies/${name_input_fasta}+samples_$divergence.l${len}.fa.gz;
	    for l in 0 100000 400000 800000; do
...
do
	...
	for aligner in wfmash; do
	  ...
	  for idx_len in ${!lengths[*]}; do
	    ...
	    echo $path_vcf_gz;
	    vg deconstruct -e -a -p $header_input_fasta -A sampleA#$header_input_fasta $path_gfa -t $threads | sed 's/#$header_input_fasta:/#${header_input_fasta}_/g' | bgzip -c > $path_vcf_gz && tabix $path_vcf_gz;
	  done;
	done;
